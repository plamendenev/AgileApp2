...
public class JUnit_TestPageVL_GOTO extends InstrumentationTestCase {
	...
	public void testActivities() throws Exception {
		Button button = (Button) app.findViewById(R.id.btn_goto);
...
//		Log.e("DEBUG 124", info.topActivity.getClassName());
		assertEquals(Calender.class.getName(), this.getLastActivityClassName());
		assertEquals(Calender.class.getName(), info.topActivity.getClassName());
		Activity cal = this.getLastActivity();
		Button button2 = (Button) cal.findViewById(R.id.cal10);
		assertNotNull(button2);
		TouchUtils.clickView(this, button2);
		info = manager.getRunningTasks(1).get(0);
		assertEquals(TestPagevl.class.getName(), info.topActivity.getClassName());
		assertEquals(9, ((TestPagevl)app).click);
	}
