...
public class TestScore extends Activity {
	...
	void display(String datetime, String timetaken, String score) {
		...
		divider.setLayoutParams(new TableLayout.LayoutParams(DIVIDER_SIZE, android.view.ViewGroup.LayoutParams.MATCH_PARENT));
		divider.setBackgroundColor(Color.BLACK);
		tr2.addView(divider, new TableRow.LayoutParams(DIVIDER_SIZE, android.view.ViewGroup.LayoutParams.MATCH_PARENT));
		TextView lbltt = new TextView(this);
		lbltt.setText("" + timetaken);
		lbltt.setTextColor(Color.BLACK);
		lbltt.setTextSize(16);
		...
		divider.setBackgroundColor(Color.BLACK);
		tr2.addView(divider, new TableRow.LayoutParams(DIVIDER_SIZE, android.view.ViewGroup.LayoutParams.MATCH_PARENT));
		prodlist.addView(tr2, new TableLayout.LayoutParams(android.view.ViewGroup.LayoutParams.WRAP_CONTENT, android.view.ViewGroup.LayoutParams.WRAP_CONTENT));
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// int j = 0;
		List<ModelScoreBoard> quants = db.getAllsbtable(cat);
		if (quants.isEmpty()) {
		...
		btn_tutorial.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				List<ModelScoreBoard> quants1 = db.getAllsbtable(cat);
				if (quants1.isEmpty()) {

				} else {

					for (ModelScoreBoard cn1 : quants1) {
						db.deletesb(cn1);
