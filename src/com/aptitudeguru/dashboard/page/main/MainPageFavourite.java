...
public class MainPageFavourite extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		List<TestModelFavourite> f6 = db.getAllFav();
		if (f6.isEmpty()) {
			...
			List<TestModelFavourite> f = db.getAllFav();
			int count = 1;
			if (f.isEmpty())
				count = 0;
			if (count != 0) {
				for (TestModelFavourite cn : f) {
					id = cn.getID();
					a[index++] = id;
				...
				TestModelFavourite f1 = db.getFav(id1);
				...
				btn_next.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View view) {
						if (current == (index - 1)) {
							...
							current = current + 1;
							int val = a[current];
							TestModelFavourite f3 = db.getFav(val);
							ques = f3.getQues();
							op1 = f3.getOption1();
							op2 = f3.getOption2();
							op3 = f3.getOption3();
			...
			removefav.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View view) {
					...
					alertDialogBuilder.setMessage("Do you want to remove this question from favourite?").setCancelable(false).setPositiveButton("Yes", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface dialog, int id) {
							List<TestModelFavourite> f = db.getAllFav();
							if (f.isEmpty()) {
								Toast.makeText(getApplicationContext(), "No Questions To Remove", Toast.LENGTH_SHORT).show();
							} else {
								int val = a[current];
