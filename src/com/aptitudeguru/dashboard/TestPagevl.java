...
public class TestPagevl extends Activity implements OnClickListener {
	...
	private long starttime1 = startTime;
...
//	public AlertDialog.Builder alertDialogBuilder;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if(bundle.getString("test")!=null) {
			if(bundle.getString("test").equals("timeouttest")) {
				testing = "timeouttest";
				startTime = 5 * 1000;
		...
		// set dialog message
		alertDialogBuilder.setMessage("Start Test?").setCancelable(false).setPositiveButton("Yes", new DialogInterface.OnClickListener() {
			@Override
		...
		// show it
		//TEST!!!!!!!
		if(getIntent().getExtras().getString("test") == null) {
			alertDialog.show();			
		}
...
//
//				int val = a[click];
//				TestModelVL q = db.getVL(val, cat);
...
//				String sol = q.getSol();
//				db.addFav(new TestModelFavourite(ques, op1, op2, op3, op4, sol));
//
...
//		});
		...
		int g = 0;
		List<TestModelVL> quants = db.getAllVL(cat);
		...
		TestModelVL q = db.getVL(initial[count], cat);
	...
	public class MyCountDownTimer extends CountDownTimer {
		...
		public void onFinish() {
			...
			AlertDialog alertDialog = new AlertDialog.Builder(TestPagevl.this).create();
