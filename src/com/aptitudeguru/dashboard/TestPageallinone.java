...
public class TestPageallinone extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// set dialog message
		alertDialogBuilder.setMessage(" Start Test With Negative Marking (0.25)?").setCancelable(false).setPositiveButton("Yes", new DialogInterface.OnClickListener() {
			@Override
		...
		if (sel[0] == 1) {
			...
			// g++;
			ul++;
			List<TestModelQuants> quants = db.getAllQuants();
			int ids[] = new int[quants.size()];
			int index1 = 0;
			if (quants.isEmpty()) {
			} else {
				for (TestModelQuants cn : quants) {
					ids[index1++] = cn.getID();
			...
			for (int i = 0; i < 10; i++) {
				...
				TestModelQuants cn = db.getQuants(ids[count]);
		...
		if (sel[1] == 1) {
			...
			// g++;
			ul++;
			List<TestModelC> quants = db.getAllC();
			int ids[] = new int[quants.size()];
			int index1 = 0;
			if (quants.isEmpty())
				Log.d("empty", "empty");
			else {
				for (TestModelC cn : quants) {
					ids[index1++] = cn.getID();
			...
			for (int i = 0; i < 10; i++) {
				...
				count = ((count + count1) % 200) + 1;
				Log.d("c count", count + "");
				TestModelC cn = db.getC(ids[count]);
		...
		if (sel[2] == 1) {
			...
			List<TestModelCpp> quants = db.getAllCpp();
			int ids[] = new int[quants.size()];
			int index1 = 0;
			if (quants.isEmpty())
				Log.d("empty", "empty");
			else {
				for (TestModelCpp cn : quants) {
					ids[index1++] = cn.getID();
			...
			for (int i = 0; i < 10; i++) {
				...
				TestModelCpp cn = db.getCpp(ids[count]);
		...
		if (sel[3] == 1) {
			...
			// {
			// g++;
			List<TestModelJava> quants = db.getAllJava();
			int ids[] = new int[quants.size()];
			int index1 = 0;
			if (quants.isEmpty())
				Log.d("empty", "empty");
			else {
				for (TestModelJava cn : quants) {
					ids[index1++] = cn.getID();
			...
			for (int i = 0; i < 10; i++) {
				...
				TestModelJava cn = db.getJava(ids[count]);
		...
		if (sel[4] == 1) {
			...
			// g++;
			ul++;
			List<TestModelHTML> quants = db.getAllHTML();
			int ids[] = new int[quants.size()];
			int index1 = 0;
			if (quants.isEmpty())
				Log.d("empty", "empty");
			else {
				for (TestModelHTML cn : quants) {
					ids[index1++] = cn.getID();
			...
			for (int i = 0; i < 10; i++) {
				...
				TestModelHTML cn = db.getHTML(ids[count]);
		...
		if (sel[5] == 1) {
			...
			// g++;
			ul++;
			List<TestModelVL> quants = db.getAllVL();
			int ids[] = new int[quants.size()];
			int index1 = 0;
			if (quants.isEmpty())
				Log.d("empty", "empty");
			else {
				for (TestModelVL cn : quants) {
					ids[index1++] = cn.getID();
			...
			for (int i = 0; i < 10; i++) {
				...
				TestModelVL cn = db.getVL(ids[count]);
		...
		if (sel[6] == 1) {
			...
			// g++;
			ul++;
			List<TestModelOS> quants = db.getAllOS();
			int ids[] = new int[quants.size()];
			int index1 = 0;
			if (quants.isEmpty())
				Log.d("empty", "empty");
			else {
				for (TestModelOS cn : quants) {
					ids[index1++] = cn.getID();
			...
			for (int i = 0; i < 10; i++) {
				...
				TestModelOS cn = db.getOS(ids[count]);
		...
		if (sel[7] == 1) {
			...
			// g++;
			ul++;
			List<TestModelDBMS> quants = db.getAllDBMS();
			int ids[] = new int[quants.size()];
			int index1 = 0;
			if (quants.isEmpty())
				Log.d("empty", "empty");
			else {
				for (TestModelDBMS cn : quants) {
					ids[index1++] = cn.getID();
			...
			for (int i = 0; i < 10; i++) {
				...
				TestModelDBMS cn = db.getDBMS(ids[count]);
		...
		if (sel[8] == 1) {
			...
			// g++;
			ul++;
			List<TestModelDSA> quants = db.getAllDSA();
			int ids[] = new int[quants.size()];
			int index1 = 0;
			if (quants.isEmpty())
				Log.d("empty", "empty");
			else {
				for (TestModelDSA cn : quants) {
					ids[index1++] = cn.getID();
			...
			for (int i = 0; i < 10; i++) {
				...
				TestModelDSA cn = db.getDSA(ids[count]);
		...
		TestModelQuants q = db.getQuants(5, "q1");
	...
	public class MyCountDownTimer extends CountDownTimer {
		...
		public void onFinish() {
			...
			AlertDialog alertDialog = new AlertDialog.Builder(TestPageallinone.this).create();
